...
static Napi::Value pad(const Napi::CallbackInfo& info) {
	...
	if (info.Length() != 3) {
		Napi::TypeError::New(info.Env(), "`pad` expects exactly 2 args").ThrowAsJavaScriptException();
		return env.Null();
	}
	return env.Null();
}
...
static Napi::Value log(const Napi::CallbackInfo& info) {
	Napi::Env env = info.Env();
	if (info.Length() != 1) {
		Napi::TypeError::New(info.Env(), "`log` expects exactly 2 args").ThrowAsJavaScriptException();
		return env.Null();
	}
	return env.Null();
}
...
static Napi::Value sigmoid(const Napi::CallbackInfo& info) {
	Napi::Env env = info.Env();
	if (info.Length() != 1) {
		Napi::TypeError::New(info.Env(), "`sigmoid` expects exactly 2 args").ThrowAsJavaScriptException();
		return env.Null();
	}
	return env.Null();
}
...
static Napi::Value isnan(const Napi::CallbackInfo& info) {
	Napi::Env env = info.Env();
	if (info.Length() != 1) {
		Napi::TypeError::New(info.Env(), "`isnan` expects exactly 2 args").ThrowAsJavaScriptException();
		return env.Null();
	}
	return env.Null();
}
...
static Napi::Value argmin(const Napi::CallbackInfo& info) {
	Napi::Env env = info.Env();
	if (info.Length() != 3) {
		Napi::TypeError::New(info.Env(), "`argmin` expects exactly 2 args").ThrowAsJavaScriptException();
		return env.Null();
	}
	return env.Null();
}
...
static Napi::Value all(const Napi::CallbackInfo& info) {
	Napi::Env env = info.Env();
	if (info.Length() != 3) {
		Napi::TypeError::New(info.Env(), "`all` expects exactly 2 args").ThrowAsJavaScriptException();
		return env.Null();
	}
	return env.Null();
}
...
static Napi::Value tanh(const Napi::CallbackInfo& info) {
	Napi::Env env = info.Env();
	if (info.Length() != 1) {
		Napi::TypeError::New(info.Env(), "`tanh` expects exactly 2 args").ThrowAsJavaScriptException();
		return env.Null();
	}
	return env.Null();
}
...
static Napi::Value floor(const Napi::CallbackInfo& info) {
	Napi::Env env = info.Env();
	if (info.Length() != 1) {
		Napi::TypeError::New(info.Env(), "`floor` expects exactly 2 args").ThrowAsJavaScriptException();
		return env.Null();
	}
	return env.Null();
}
...
static Napi::Value argsort(const Napi::CallbackInfo& info) {
	Napi::Env env = info.Env();
	if (info.Length() != 3) {
		Napi::TypeError::New(info.Env(), "`argsort` expects exactly 2 args").ThrowAsJavaScriptException();
		return env.Null();
	}
	return env.Null();
}
...
static Napi::Value sqrt(const Napi::CallbackInfo& info) {
	Napi::Env env = info.Env();
	if (info.Length() != 1) {
		Napi::TypeError::New(info.Env(), "`sqrt` expects exactly 2 args").ThrowAsJavaScriptException();
		return env.Null();
	}
	return env.Null();
}
...
static Napi::Value rint(const Napi::CallbackInfo& info) {
	Napi::Env env = info.Env();
	if (info.Length() != 1) {
		Napi::TypeError::New(info.Env(), "`rint` expects exactly 2 args").ThrowAsJavaScriptException();
		return env.Null();
	}
	return env.Null();
}
...
static Napi::Value roll(const Napi::CallbackInfo& info) {
	Napi::Env env = info.Env();
	if (info.Length() != 3) {
		Napi::TypeError::New(info.Env(), "`roll` expects exactly 2 args").ThrowAsJavaScriptException();
		return env.Null();
	}
	return env.Null();
}
...
static Napi::Value countNonzero(const Napi::CallbackInfo& info) {
	Napi::Env env = info.Env();
	if (info.Length() != 3) {
		Napi::TypeError::New(info.Env(), "`countNonzero` expects exactly 2 args").ThrowAsJavaScriptException();
		return env.Null();
	}
	return env.Null();
}
...
static Napi::Value sign(const Napi::CallbackInfo& info) {
	Napi::Env env = info.Env();
	if (info.Length() != 1) {
		Napi::TypeError::New(info.Env(), "`sign` expects exactly 2 args").ThrowAsJavaScriptException();
		return env.Null();
	}
	return env.Null();
}
...
static Napi::Value median(const Napi::CallbackInfo& info) {
	Napi::Env env = info.Env();
	if (info.Length() != 3) {
		Napi::TypeError::New(info.Env(), "`median` expects exactly 2 args").ThrowAsJavaScriptException();
		return env.Null();
	}
	return env.Null();
}
...
static Napi::Value any(const Napi::CallbackInfo& info) {
	Napi::Env env = info.Env();
	if (info.Length() != 3) {
		Napi::TypeError::New(info.Env(), "`any` expects exactly 2 args").ThrowAsJavaScriptException();
		return env.Null();
	}
	return env.Null();
}
...
static Napi::Value sin(const Napi::CallbackInfo& info) {
	Napi::Env env = info.Env();
	if (info.Length() != 1) {
		Napi::TypeError::New(info.Env(), "`sin` expects exactly 2 args").ThrowAsJavaScriptException();
		return env.Null();
	}
	return env.Null();
}
...
static Napi::Value matmul(const Napi::CallbackInfo& info) {
	Napi::Env env = info.Env();
	if (info.Length() != 4) {
		Napi::TypeError::New(info.Env(), "`matmul` expects exactly 2 args").ThrowAsJavaScriptException();
		return env.Null();
	}
	return env.Null();
}
...
static Napi::Value norm(const Napi::CallbackInfo& info) {
	Napi::Env env = info.Env();
	if (info.Length() != 4) {
		Napi::TypeError::New(info.Env(), "`norm` expects exactly 2 args").ThrowAsJavaScriptException();
		return env.Null();
	}
	return env.Null();
}
...
static Napi::Value erf(const Napi::CallbackInfo& info) {
	Napi::Env env = info.Env();
	if (info.Length() != 1) {
		Napi::TypeError::New(info.Env(), "`erf` expects exactly 2 args").ThrowAsJavaScriptException();
		return env.Null();
	}
	return env.Null();
}
...
static Napi::Value std(const Napi::CallbackInfo& info) {
	Napi::Env env = info.Env();
	if (info.Length() != 3) {
		Napi::TypeError::New(info.Env(), "`std` expects exactly 2 args").ThrowAsJavaScriptException();
		return env.Null();
	}
	return env.Null();
}
...
static Napi::Value negative(const Napi::CallbackInfo& info) {
	Napi::Env env = info.Env();
	if (info.Length() != 1) {
		Napi::TypeError::New(info.Env(), "`negative` expects exactly 2 args").ThrowAsJavaScriptException();
		return env.Null();
	}
	return env.Null();
}
...
static Napi::Value amin(const Napi::CallbackInfo& info) {
	Napi::Env env = info.Env();
	if (info.Length() != 3) {
		Napi::TypeError::New(info.Env(), "`amin` expects exactly 2 args").ThrowAsJavaScriptException();
		return env.Null();
	}
	return env.Null();
}
...
static Napi::Value tril(const Napi::CallbackInfo& info) {
	Napi::Env env = info.Env();
	if (info.Length() != 1) {
		Napi::TypeError::New(info.Env(), "`tril` expects exactly 2 args").ThrowAsJavaScriptException();
		return env.Null();
	}
	return env.Null();
}
...
static Napi::Value argmax(const Napi::CallbackInfo& info) {
	Napi::Env env = info.Env();
	if (info.Length() != 3) {
		Napi::TypeError::New(info.Env(), "`argmax` expects exactly 2 args").ThrowAsJavaScriptException();
		return env.Null();
	}
	return env.Null();
}
...
static Napi::Value scalar(const Napi::CallbackInfo& info) {
	Napi::Env env = info.Env();
	if (info.Length() != 0) {
		Napi::TypeError::New(info.Env(), "`scalar` expects exactly 2 args").ThrowAsJavaScriptException();
		return env.Null();
	}
	return env.Null();
}
...
static Napi::Value triu(const Napi::CallbackInfo& info) {
	Napi::Env env = info.Env();
	if (info.Length() != 1) {
		Napi::TypeError::New(info.Env(), "`triu` expects exactly 2 args").ThrowAsJavaScriptException();
		return env.Null();
	}
	return env.Null();
}
...
static Napi::Value mean(const Napi::CallbackInfo& info) {
	Napi::Env env = info.Env();
	if (info.Length() != 3) {
		Napi::TypeError::New(info.Env(), "`mean` expects exactly 2 args").ThrowAsJavaScriptException();
		return env.Null();
	}
	return env.Null();
}
...
static Napi::Value cos(const Napi::CallbackInfo& info) {
	Napi::Env env = info.Env();
	if (info.Length() != 1) {
		Napi::TypeError::New(info.Env(), "`cos` expects exactly 2 args").ThrowAsJavaScriptException();
		return env.Null();
	}
	return env.Null();
}
...
static Napi::Value ceil(const Napi::CallbackInfo& info) {
	Napi::Env env = info.Env();
	if (info.Length() != 1) {
		Napi::TypeError::New(info.Env(), "`ceil` expects exactly 2 args").ThrowAsJavaScriptException();
		return env.Null();
	}
	return env.Null();
}
...
static Napi::Value absolute(const Napi::CallbackInfo& info) {
	Napi::Env env = info.Env();
	if (info.Length() != 1) {
		Napi::TypeError::New(info.Env(), "`absolute` expects exactly 2 args").ThrowAsJavaScriptException();
		return env.Null();
	}
	return env.Null();
}
...
static Napi::Value sum(const Napi::CallbackInfo& info) {
	Napi::Env env = info.Env();
	if (info.Length() != 3) {
		Napi::TypeError::New(info.Env(), "`sum` expects exactly 2 args").ThrowAsJavaScriptException();
		return env.Null();
	}
	return env.Null();
}
...
static Napi::Value isinf(const Napi::CallbackInfo& info) {
	Napi::Env env = info.Env();
	if (info.Length() != 1) {
		Napi::TypeError::New(info.Env(), "`isinf` expects exactly 2 args").ThrowAsJavaScriptException();
		return env.Null();
	}
	return env.Null();
}
...
static Napi::Value logicalNot(const Napi::CallbackInfo& info) {
	Napi::Env env = info.Env();
	if (info.Length() != 1) {
		Napi::TypeError::New(info.Env(), "`logicalNot` expects exactly 2 args").ThrowAsJavaScriptException();
		return env.Null();
	}
	return env.Null();
}
...
static Napi::Value exp(const Napi::CallbackInfo& info) {
	Napi::Env env = info.Env();
	if (info.Length() != 1) {
		Napi::TypeError::New(info.Env(), "`exp` expects exactly 2 args").ThrowAsJavaScriptException();
		return env.Null();
	}
	return env.Null();
}
...
static Napi::Value log1p(const Napi::CallbackInfo& info) {
	Napi::Env env = info.Env();
	if (info.Length() != 1) {
		Napi::TypeError::New(info.Env(), "`log1p` expects exactly 2 args").ThrowAsJavaScriptException();
		return env.Null();
	}
	return env.Null();
}
...
static Napi::Value nonzero(const Napi::CallbackInfo& info) {
	Napi::Env env = info.Env();
	if (info.Length() != 1) {
		Napi::TypeError::New(info.Env(), "`nonzero` expects exactly 2 args").ThrowAsJavaScriptException();
		return env.Null();
	}
	return env.Null();
}
...
static Napi::Value amax(const Napi::CallbackInfo& info) {
	Napi::Env env = info.Env();
	if (info.Length() != 3) {
		Napi::TypeError::New(info.Env(), "`amax` expects exactly 2 args").ThrowAsJavaScriptException();
		return env.Null();
	}
	return env.Null();
}
...
static Napi::Value var(const Napi::CallbackInfo& info) {
	Napi::Env env = info.Env();
	if (info.Length() != 4) {
		Napi::TypeError::New(info.Env(), "`var` expects exactly 2 args").ThrowAsJavaScriptException();
		return env.Null();
	}
	return env.Null();
}
