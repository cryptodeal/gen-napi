...
static Napi::External<fl::Tensor> WrapTensor(Napi::Env env, void* t) {
  auto* tensor = reinterpret_cast<fl::Tensor*>(t);
  Napi::External<fl::Tensor> wrapped =
      Napi::External<fl::Tensor>::New(env, tensor, [](Napi::Env env, void* t) {
        auto* tensor = reinterpret_cast<fl::Tensor*>(t);
...
static Napi::Value _clip(const Napi::CallbackInfo& info) {
  ...
  if (tensor_obj.InstanceOf(Tensor::constructor->Value()) &&
    ...
    auto wrapped = WrapTensor(env, out);
...
Napi::Value Tensor::copy(const Napi::CallbackInfo& info) {
  ...
  g_bytes_used += _res.bytes();
  auto* out = new fl::Tensor(_res);
  auto _wrapped = WrapTensor(env, out);
  ;
...
static Napi::Value _rand(const Napi::CallbackInfo& info) {
  ...
  auto wrappedTensor = WrapTensor(env, tensor);
