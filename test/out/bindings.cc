...
static Napi::Value _amax(const Napi::CallbackInfo& info) {
  ...
  if (input_obj.InstanceOf(Tensor::constructor->Value())) {
    ...
    Napi::External<fl::Tensor> _wrapped = ExternalizeTensor(env, out);
...
Tensor::Tensor(const Napi::CallbackInfo& info) : ObjectWrap(info) {
    ...
    switch (arrayType) {
      case napi_float32_array: {
        ...
        Napi::External<fl::Tensor> wrapped =
            ExternalizeTensor<fl::Tensor>(env, t);
  ...
  if (info[0].IsNumber()) {
    ...
    g_bytes_used += t->bytes();
    Napi::External<fl::Tensor> wrapped = ExternalizeTensor<fl::Tensor>(env, t);
    this->_tensor = wrapped.Data();
    return;
  }
}
