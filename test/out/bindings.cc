...
static Napi::Value _iota(const Napi::CallbackInfo& info) {
	...
	if (!info[1].IsArray()) {
		...
		::Tensor _res;
		_res = ::iota(::Shape(dims), ::Shape(tileDims), type);
		g_bytes_used += _res.bytes();
		auto* out = new ::Tensor(_res);
		auto wrapped = Napi::External<::Tensor>::New(env, out);
		Napi::Value wrapped_out = Tensor::constructor->New({wrapped});
		return wrapped_out;
}
