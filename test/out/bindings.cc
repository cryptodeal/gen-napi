...
template <typename T>
static inline std::vector<T> jsArrayToVector(Napi::Env env,
                                             Napi::Array arr,
                                             bool reverse,
                                             int invert) {
  std::vector<T> out;
  ...
  for (size_t i = 0; i < len; ++i) {
    auto idx = reverse ? len - i - 1 : i;
    Napi::Value val = arr[idx];
    auto v = reinterpret_cast<const T>(val.As<Napi::Number>().Int64Value());
    if (invert && v < 0) {
      v = -v - 1;
    } else if (invert) {
      v = invert - v - 1;
...
static Napi::Value _iota(const Napi::CallbackInfo& info) {
  ...
  auto dims = jsArrayToVector<long long>(info[0].As<Napi::Array>(), g_row_major,
                                         false, env);
  if (!info[1].IsArray()) {
  ...
  auto tileDims = jsArrayToVector<long long>(info[1].As<Napi::Array>(),
                                             g_row_major, false, env);
  if (!info[2].IsNumber()) {
...
static Napi::Value _full(const Napi::CallbackInfo& info) {
  ...
  std::vector<long long> dims = jsArrayToVector<long long>(
...
static Napi::Value _tile(const Napi::CallbackInfo& info) {
  ...
  auto shape = jsArrayToVector<long long>(info[1].As<Napi::Array>(),
                                          g_row_major, false, env);
  fl::Tensor _res;
