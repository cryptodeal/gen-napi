...
static inline void DestroyTensor(Napi::Env env, void* ptr) {
...
static inline Napi::External<fl::Tensor> WrapTensor(Napi::Env env,
                                                    fl::Tensor* t) {
  return Napi::External<fl::Tensor>::New(env, t, DestroyTensor);
}
template <typename T>
...
static Napi::Value _logicalNot(const Napi::CallbackInfo& info) {
  ...
  if (tensor_obj.InstanceOf(Tensor::constructor->Value())) {
    ...
    Napi::External<fl::Tensor> _wrapped = WrapTensor(env, out);
...
static Napi::Value _full(const Napi::CallbackInfo& info) {
  ...
  g_bytes_used += _res.bytes();
  auto* out = new fl::Tensor(_res);
  Napi::External<fl::Tensor> _wrapped =
      Napi::External<fl::Tensor>::New(env, out, [](Napi::Env env, void* ptr) {
        auto* val = static_cast<fl::Tensor*>(ptr);
        if (val->hasAdapter()) {
          g_bytes_used -= val->bytes();
        }
...
Tensor::Tensor(const Napi::CallbackInfo& info) : ObjectWrap(info) {
  ...
  if (info[0].IsTypedArray()) {
    ...
    switch (arrayType) {
      case napi_float32_array: {
        ...
        g_bytes_used += t->bytes();
        Napi::External<fl::Tensor> wrapped = Napi::External<fl::Tensor>::New(
            env, t, [](Napi::Env env, void* ptr) {
              auto* val = static_cast<fl::Tensor*>(ptr);
              if (val->hasAdapter()) {
                g_bytes_used -= val->bytes();
              }
  ...
  if (info[0].IsNumber()) {
    ...
    Napi::External<fl::Tensor> wrapped =
        Napi::External<fl::Tensor>::New(env, t, [](Napi::Env env, void* ptr) {
          auto* val = static_cast<fl::Tensor*>(ptr);
          if (val->hasAdapter()) {
            g_bytes_used -= val->bytes();
          }
...
void Tensor::dispose(const Napi::CallbackInfo& info) {
  auto& tensor = *static_cast<fl::Tensor*>(this->_tensor);
...
static Napi::Value _rand(const Napi::CallbackInfo& info) {
  ...
  auto wrapped = Napi::External<fl::Tensor>::New(
      env, tensor, [](Napi::Env env, void* ptr) {
        auto* val = static_cast<fl::Tensor*>(ptr);
        if (val->hasAdapter()) {
          g_bytes_used -= val->bytes();
        }
