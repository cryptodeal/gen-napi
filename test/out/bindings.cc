...
static Napi::Value _max(const Napi::CallbackInfo& info) {
  Napi::Env env = info.Env();
  if (info.Length() != 5) {
  ...
  if (!info[4].IsBoolean()) {
    Napi::TypeError::New(env, "`max` expects args[4] to be typeof `boolean`)")
        .ThrowAsJavaScriptException();
    return env.Undefined();
  }
  bool keepDims = info[4].As<Napi::Boolean>().Value();
  void _res;
  _res = fl::max(*(values), *(indices), *(input), axis, keepDims);
  auto* out = new fl::void(_res);
  Napi::External<fl::void> _external_out = Externalizevoid(env, out);
  return _external_out;
}
...
static Napi::Value _allClose(const Napi::CallbackInfo& info) {
  ...
  auto* out = new fl::bool(_res);
  Napi::External<fl::bool> _external_out = Externalizebool(env, out);
  return _external_out;
}
...
static Napi::Value _areTensorTypesEqual(const Napi::CallbackInfo& info) {
  ...
  bool _res;
  _res = fl::areTensorTypesEqual(*(a), *(b));
  auto* out = new fl::bool(_res);
  Napi::External<fl::bool> _external_out = Externalizebool(env, out);
  return _external_out;
}
...
static Napi::Value _min(const Napi::CallbackInfo& info) {
  Napi::Env env = info.Env();
  if (info.Length() != 5) {
  ...
  unsigned axis =
      static_cast<unsigned>(info[3].As<Napi::Number>().Uint32Value());
  if (!info[4].IsBoolean()) {
    Napi::TypeError::New(env, "`min` expects args[4] to be typeof `boolean`)")
        .ThrowAsJavaScriptException();
    return env.Undefined();
  }
  bool keepDims = info[4].As<Napi::Boolean>().Value();
  void _res;
  _res = fl::min(*(values), *(indices), *(input), axis, keepDims);
  auto* out = new fl::void(_res);
  Napi::External<fl::void> _external_out = Externalizevoid(env, out);
  return _external_out;
}
...
static Napi::Value _print(const Napi::CallbackInfo& info) {
  Napi::Env env = info.Env();
  if (info.Length() != 1) {
    Napi::TypeError::New(env, "`print` expects exactly 1 arg")
        .ThrowAsJavaScriptException();
    return env.Undefined();
  }
  if (!info[0].IsExternal()) {
    Napi::TypeError::New(env,
                         "`print` expects args[0] to be native `Tensor` "
                         "(typeof `Napi::External<fl::Tensor>`)")
        .ThrowAsJavaScriptException();
    return env.Undefined();
  }
...
static Napi::Value _topk(const Napi::CallbackInfo& info) {
  Napi::Env env = info.Env();
  if (info.Length() != 6) {
    Napi::TypeError::New(env, "`topk` expects exactly 6 args")
        .ThrowAsJavaScriptException();
    return env.Undefined();
  }
  if (!info[0].IsExternal()) {
    Napi::TypeError::New(env,
                         "`topk` expects args[0] to be native `Tensor` (typeof "
                         "`Napi::External<fl::Tensor>`)")
        .ThrowAsJavaScriptException();
    return env.Undefined();
  }
  ...
  if (!info[1].IsExternal()) {
    Napi::TypeError::New(env,
                         "`topk` expects args[1] to be native `Tensor` (typeof "
                         "`Napi::External<fl::Tensor>`)")
        .ThrowAsJavaScriptException();
    return env.Undefined();
  }
  ...
  if (!info[2].IsExternal()) {
    Napi::TypeError::New(env,
                         "`topk` expects args[2] to be native `Tensor` (typeof "
                         "`Napi::External<fl::Tensor>`)")
        .ThrowAsJavaScriptException();
    return env.Undefined();
  }
  fl::Tensor* input = UnExternalize<fl::Tensor>(info[2]);
  if (!info[3].IsNumber()) {
    Napi::TypeError::New(env, "`topk` expects args[3] to be typeof `number`)")
        .ThrowAsJavaScriptException();
    return env.Undefined();
  }
  unsigned k = static_cast<unsigned>(info[3].As<Napi::Number>().Uint32Value());
  if (!info[4].IsNumber()) {
    Napi::TypeError::New(env, "`topk` expects args[4] to be typeof `number`)")
        .ThrowAsJavaScriptException();
    return env.Undefined();
  }
  void _res;
  _res = fl::topk(*(values), *(indices), *(input), k, fl::Dim(axis), sortMode);
  auto* out = new fl::void(_res);
  Napi::External<fl::void> _external_out = Externalizevoid(env, out);
  return _external_out;
}
...
Napi::Object Init(Napi::Env env, Napi::Object exports) {
  ...
  exports.Set(Napi::String::New(env, "_print"),
              Napi::Function::New(env, _print));
  ...
  exports.Set(Napi::String::New(env, "_topk"), Napi::Function::New(env, _topk));
