...
static Napi::Value _all(const Napi::CallbackInfo& info) {
	...
	for (auto i = 0; i < len_axes; ++i) {
		if (!info[1].As<Napi::Array>().Get(i).IsNumber()) {
			Napi::TypeError::New(info.Env(), "`all` expects args[1][i] to be typeof `number`").ThrowAsJavaScriptException();
			return env.Null();
		}
	}
	if (!info[2].IsBoolean()) {
...
static Napi::Value _std(const Napi::CallbackInfo& info) {
	...
	for (auto i = 0; i < len_axes; ++i) {
		if (!info[1].As<Napi::Array>().Get(i).IsNumber()) {
			Napi::TypeError::New(info.Env(), "`std` expects args[1][i] to be typeof `number`").ThrowAsJavaScriptException();
			return env.Null();
		}
	}
	if (!info[2].IsBoolean()) {
...
static Napi::Value _norm(const Napi::CallbackInfo& info) {
	...
	for (auto i = 0; i < len_axes; ++i) {
		if (!info[1].As<Napi::Array>().Get(i).IsNumber()) {
			Napi::TypeError::New(info.Env(), "`norm` expects args[1][i] to be typeof `number`").ThrowAsJavaScriptException();
			return env.Null();
		}
	}
	if (!info[2].IsNumber()) {
...
static Napi::Value _any(const Napi::CallbackInfo& info) {
	...
	for (auto i = 0; i < len_axes; ++i) {
		if (!info[1].As<Napi::Array>().Get(i).IsNumber()) {
			Napi::TypeError::New(info.Env(), "`any` expects args[1][i] to be typeof `number`").ThrowAsJavaScriptException();
			return env.Null();
		}
	}
	if (!info[2].IsBoolean()) {
...
static Napi::Value _median(const Napi::CallbackInfo& info) {
	...
	for (auto i = 0; i < len_axes; ++i) {
		if (!info[1].As<Napi::Array>().Get(i).IsNumber()) {
			Napi::TypeError::New(info.Env(), "`median` expects args[1][i] to be typeof `number`").ThrowAsJavaScriptException();
			return env.Null();
		}
	}
	if (!info[2].IsBoolean()) {
...
static Napi::Value _countNonzero(const Napi::CallbackInfo& info) {
	...
	for (auto i = 0; i < len_axes; ++i) {
		if (!info[1].As<Napi::Array>().Get(i).IsNumber()) {
			Napi::TypeError::New(info.Env(), "`countNonzero` expects args[1][i] to be typeof `number`").ThrowAsJavaScriptException();
			return env.Null();
		}
	}
	if (!info[2].IsBoolean()) {
...
static Napi::Value _sum(const Napi::CallbackInfo& info) {
	...
	for (auto i = 0; i < len_axes; ++i) {
		if (!info[1].As<Napi::Array>().Get(i).IsNumber()) {
			Napi::TypeError::New(info.Env(), "`sum` expects args[1][i] to be typeof `number`").ThrowAsJavaScriptException();
			return env.Null();
		}
	}
	if (!info[2].IsBoolean()) {
...
static Napi::Value _amin(const Napi::CallbackInfo& info) {
	...
	for (auto i = 0; i < len_axes; ++i) {
		if (!info[1].As<Napi::Array>().Get(i).IsNumber()) {
			Napi::TypeError::New(info.Env(), "`amin` expects args[1][i] to be typeof `number`").ThrowAsJavaScriptException();
			return env.Null();
		}
	}
	if (!info[2].IsBoolean()) {
...
static Napi::Value _var(const Napi::CallbackInfo& info) {
	...
	for (auto i = 0; i < len_axes; ++i) {
		if (!info[1].As<Napi::Array>().Get(i).IsNumber()) {
			Napi::TypeError::New(info.Env(), "`var` expects args[1][i] to be typeof `number`").ThrowAsJavaScriptException();
			return env.Null();
		}
	}
	if (!info[2].IsBoolean()) {
...
static Napi::Value _concatenate(const Napi::CallbackInfo& info) {
	...
	for (auto i = 0; i < len_tensors; ++i) {
		if (!info[0].As<Napi::Array>().Get(i).IsObject()) {
			Napi::TypeError::New(info.Env(), "`concatenate` expects args[0][i] to be typeof `Tensor`").ThrowAsJavaScriptException();
			return env.Null();
		}
	}
	if (!info[1].IsNumber()) {
...
static Napi::Value _amax(const Napi::CallbackInfo& info) {
	...
	for (auto i = 0; i < len_axes; ++i) {
		if (!info[1].As<Napi::Array>().Get(i).IsNumber()) {
			Napi::TypeError::New(info.Env(), "`amax` expects args[1][i] to be typeof `number`").ThrowAsJavaScriptException();
			return env.Null();
		}
	}
	if (!info[2].IsBoolean()) {
...
static Napi::Value _mean(const Napi::CallbackInfo& info) {
	...
	for (auto i = 0; i < len_axes; ++i) {
		if (!info[1].As<Napi::Array>().Get(i).IsNumber()) {
			Napi::TypeError::New(info.Env(), "`mean` expects args[1][i] to be typeof `number`").ThrowAsJavaScriptException();
			return env.Null();
		}
	}
	if (!info[2].IsBoolean()) {
