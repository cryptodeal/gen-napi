...
static Napi::Value _countNonzero(const Napi::CallbackInfo& info) {
	Napi::Env env = info.Env();
	if (info.Length() != 5) {
		Napi::TypeError::New(info.Env(), "`countNonzero` expects exactly 5 args").ThrowAsJavaScriptException();
		return env.Null();
	}
	if (!info[0].IsObject()) {
	...
	if (input_obj.InstanceOf(Tensor::constructor->Value())) {
		...
		auto base_shape = *(input->_tensor)->_tensor->shape().get();
...
static Napi::Value _arange(const Napi::CallbackInfo& info) {
	...
	if (!info[0].IsNumber()) {
	   Napi::TypeError::New(
	       info.Env(), "`arange` expects args[0] to be typeof `number`")
	       .ThrowAsJavaScriptException();
	   return env.Null();
	 }
	 if (!info[1].IsNumber()) {
	 ...
	 if (!info[2].IsNumber()) {
	   Napi::TypeError::New(
	       info.Env(), "`arange` expects args[2] to be typeof `number`")
	       .ThrowAsJavaScriptException();
	   return env.Null();
	 }
...
static Napi::Value _matmul(const Napi::CallbackInfo& info) {
	Napi::Env env = info.Env();
	if (info.Length() != 3) {
		Napi::TypeError::New(info.Env(), "`matmul` expects exactly 3 args").ThrowAsJavaScriptException();
		return env.Null();
	}
	if (!info[0].IsObject()) {
