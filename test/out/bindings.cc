...
static Napi::Value full(const Napi::CallbackInfo& info) {
	...
	if (info.Length() != 3) {
		...
		return env.Null();
			...
			Napi::TypeError::New(info.Env(), "`full` expects args[0] to be typeof `{ }`").ThrowAsJavaScriptException();
		return env.Null();
	}
}
...
static Napi::Value reshape(const Napi::CallbackInfo& info) {
	...
	Napi::Object tensor_obj = info[0].As<Napi::Object>();
			Napi::TypeError::New(info.Env(), "`reshape` expects args[1] to be typeof `{ }`").ThrowAsJavaScriptException();
		return env.Null();
	}
	if (tensor_obj.InstanceOf(Tensor::constructor->Value())) {
		Tensor* tensor = Napi::ObjectWrap<Tensor>::Unwrap(tensor_obj);
...
static Napi::Value transpose(const Napi::CallbackInfo& info) {
	...
	Napi::Object tensor_obj = info[0].As<Napi::Object>();
			Napi::TypeError::New(info.Env(), "`transpose` expects args[1] to be typeof `{ }`").ThrowAsJavaScriptException();
		return env.Null();
	}
	if (tensor_obj.InstanceOf(Tensor::constructor->Value())) {
		Tensor* tensor = Napi::ObjectWrap<Tensor>::Unwrap(tensor_obj);
		auto axes = jsArrayArg<long long>(info[1].As<Napi::Array>(), g_row_major, info[0]->_tensor->ndim(), env);
		fl::Tensor res;
		res = fl::transpose(*(*(tensor->_tensor)->_tensor), ))
...
static Napi::Value tile(const Napi::CallbackInfo& info) {
	...
	Napi::Object tensor_obj = info[0].As<Napi::Object>();
			Napi::TypeError::New(info.Env(), "`tile` expects args[1] to be typeof `{ }`").ThrowAsJavaScriptException();
		return env.Null();
	}
	if (tensor_obj.InstanceOf(Tensor::constructor->Value())) {
		Tensor* tensor = Napi::ObjectWrap<Tensor>::Unwrap(tensor_obj);
...
static Napi::Value iota(const Napi::CallbackInfo& info) {
	...
	if (info.Length() != 3) {
		...
		return env.Null();
			...
			Napi::TypeError::New(info.Env(), "`iota` expects args[0] to be typeof `{ }`").ThrowAsJavaScriptException();
		return env.Null();
	}
			Napi::TypeError::New(info.Env(), "`iota` expects args[1] to be typeof `{ }`").ThrowAsJavaScriptException();
		return env.Null();
	}
}
