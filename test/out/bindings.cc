...
static Napi::Value _argmin(const Napi::CallbackInfo& info) {
  ...
  boolbool keepDims = info[2].As<Napi::Boolean>().Value();
...
static Napi::Value _norm(const Napi::CallbackInfo& info) {
  ...
  doubledouble p = info[2].As<Napi::Number>().DoubleValue();
  if (!info[3].IsBoolean()) {
    Napi::TypeError::New(env, "`norm` expects args[3] to be typeof `boolean`)")
        .ThrowAsJavaScriptException();
    return env.Undefined();
  }
  boolbool keepDims = info[3].As<Napi::Boolean>().Value();
...
static Napi::Value _var(const Napi::CallbackInfo& info) {
  ...
  boolbool bias = info[2].As<Napi::Boolean>().Value();
  if (!info[3].IsBoolean()) {
...
static Napi::Value _roll(const Napi::CallbackInfo& info) {
  ...
  intint shift = static_cast<int>(info[1].As<Napi::Number>().Int64Value());
  if (!info[2].IsNumber()) {
