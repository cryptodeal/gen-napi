...
static Napi::Value _iota(const Napi::CallbackInfo& info) {
  ...
  fl::Shape dims = fl::Shape(jsArrayToVector<long long>(
      info[0].As<Napi::Array>(), g_row_major, false));
  if (!info[1].IsArray()) {
    Napi::TypeError::New(env, "`iota` expects args[1] to be typeof `number[]`)")
        .ThrowAsJavaScriptException();
    return env.Undefined();
  }
  fl::Shape tileDims = fl::Shape(jsArrayToVector<long long>(
      info[1].As<Napi::Array>(), g_row_major, false));
  fl::dtype type;
