...
class Tensor {
 ...
 public:
  ...
  static Tensor fromVector(std::vector<T> v) {
    return Tensor({static_cast<long long>(v.size())},
                  fl::dtype_traits<T>::fl_type, v.data(), Location::Host);
  }

  template <typename T, std::size_t S>
  static Tensor fromArray(std::array<T, S> a) {
    return Tensor({static_cast<long long>(a.size())},
                  fl::dtype_traits<T>::fl_type, a.data(), Location::Host);
  }
   ...
   * @return a tensor with values and shape as given.
   */
  static Tensor fromBuffer(Shape s,
                           fl::dtype t,
                           const uint8_t* ptr,
 ...
 */
Tensor sort(const Tensor& input,
            const Dim axis,
 ...
 * @param[in] sortMode the ordering with which to sort. Defaults to ascending
 */
Tensor argsort(const Tensor& input,
               const Dim axis,
               const SortMode sortMode = SortMode::Ascending);
 ...
 */
Tensor amin(const Tensor& input,
 ...
 * @return a tensor containing the indices of the max values along each axis
 */
Tensor argmin(const Tensor& input,
              const unsigned axis,
              const bool keepDims = false);
 ...
 */
Tensor median(const Tensor& input,
              const std::vector<int>& axes = {},
              const bool keepDims = false);
 ...
 */
Tensor norm(const Tensor& input,
            const std::vector<int>& axes = {},
            double p = 2,
            const bool keepDims = false);
 ...
 */
Tensor countNonzero(const Tensor& input,
                    const std::vector<int>& axes = {},
                    const bool keepDims = false);
 ...
 * tensors
 */
bool allClose(const fl::Tensor& a,
              const fl::Tensor& b,
...
Tensor to(Tensor&& t) {
  ...
  if (t.isSparse()) {
    ...
    return Tensor(std::make_unique<T>(t.shape(), t.type(), t.device<void>(),
...
bool areTensorTypesEqual(const Tensor& a,
                         const Tensor& b,
                         const Args&... args) {
...
#define FL_TENSOR_DTYPES_MATCH_CHECK(...)                                     \
  if (!detail::areTensorTypesEqual(__VA_ARGS__)) {                            \
    throw std::invalid_argument(std::string(__func__) +                       \
                                ": tensors are not all of the same types. "); \
  }
